...
ファイルシステムとは, ファイルを管理する仕組みのことです. ファイルシステムのおかげでファイルをディレクトリにまとめたり, 
ファイルをアクセス権で保護したりできます. 利用できるファイルシステムはOSによって違います. Linuxが扱うことができる
ファイルシステムの例は以下です:
...
ファイルシステムによってデバイスデータがデバイスファイルとして管理され, そのためデバイス操作が普通のファイル操作
と同じ感覚で実行できるようになっています. Linux ではデバイスファイルは `/dev` ディレクトリに存在します.
