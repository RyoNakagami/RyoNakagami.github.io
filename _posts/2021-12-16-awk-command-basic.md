---
layout: post
title: "awkコマンドを用いた前処理の紹介"
subtitle: "awk command 1/N"
author: "Ryo"
header-style: text
header-mask: 0.0
catelog: true
...
`awk /正規表現/`で`grep 正規表現`と同じ挙動になります. 別の書き方として2で割り切れる数値が偶数であること利用して次のような書き方も可能です.
...
「別解」では、`CONDITION ? 真の場合に返す値 : 偽の場合に返す値`という三項演算子を用いて記述しています. 
awkコマンドのフロートしては、
