...
- [Kanbanをチーム開発に根付かせるためには？](#kanban%E3%82%92%E3%83%81%E3%83%BC%E3%83%A0%E9%96%8B%E7%99%BA%E3%81%AB%E6%A0%B9%E4%BB%98%E3%81%8B%E3%81%9B%E3%82%8B%E3%81%9F%E3%82%81%E3%81%AB%E3%81%AF)
  ...
  - [KanbanチケットをBackwardsに動かしてよいのか？](#kanban%E3%83%81%E3%82%B1%E3%83%83%E3%83%88%E3%82%92backwards%E3%81%AB%E5%8B%95%E3%81%8B%E3%81%97%E3%81%A6%E3%82%88%E3%81%84%E3%81%AE%E3%81%8B)
...
「Backlog」「In-Progress」「Review」「Confirmed」などとタスクが流れるステージを定義し, 
現時点で各タスクがどのステージにどれだけ滞留しているかを可視化することを通じて, チームリソース管理やタスク
進捗管理を実現する仕組みのこと.

どのステージにどの程度のタスクが滞留しているか?, どのステージに余剰人員/人員不足が発生しているか？
を判断する要請から, 各タスクにはチームメンバーが共通して理解できる**優先度**が定義されている必要がある.
...
ただし, Kanbanボードは現在進行中のタスクのみにフォーカスする = 完了タスクを振り返る仕組みではないことに留意してください.


<p class="h4"><ins>(1): Planning flexibility</ins></p>

- プロジェクトオーナーはBacklogのタスクの優先度のメンテナンスを、現在進行中のタスクに影響を与えることなく実行できる
- 開発チームはBacklogに存在するタスクのうち優先度の高いタスクにフォーカスすることができる
- タスクを完了次第, Backlogから優先度に基づき次のタスクを取り出す形でプロジェクトを進めていくことができる


<p class="h4"><ins>(2) Shortened time cycles</ins></p>

- タスクの起票からdelivery pointまでに要した時間をCycle timeという
...
- チームの効率性 & 進捗監視として以下の３つの指標を見ることが可能になる:
  1. **平均サイクル時間の減少**
  2. **ステージごとのタスク着手件数**
  3. **ステージごとの累積処理件数**
...
kanbanチームは, 通常1枚のカードに1つずつ作業項目をJob card(work itemやカードとも)に書き込む. アジャイルチームの場合, 各カードに1つのユーザーストーリーを組み込むことができる.Kanban board上にあるこれらのvisual signalは, チームメンバーが素早くチームが取り組んでいることを理解するのに役立つ効果がある.

<p class="h4"><ins>(2): Columns</ins></p>

それぞれのColumn（swimlanes）は, 「ワークフロー」と呼ばれる特定の**アクティビティ**を表している. カードは, 完了するまでワークフローを流れる. ワークフローは, Open, WIP, Review, Completeといった単純なものから, 開発フローに合わせた複雑なものまである.

<p class="h4"><ins>(3): WIP Limits </ins></p>

WIP Limitsとは, 任意の時点で1つのColumnにあるカードの最大数のこと. WIP Limits が 3 の列には, 合計weightが3以上となった瞬間に新たなカードを置くことはできなくなるなる. 
...
ワークフローの効率化は, コミットメントポイントからデリバリーポイントまでのカードをできるだけ早く進めること = リードタイムの削減が一つの重要な要素となる.
...
上記のStoriesはすべて関連しており, より大きな作業の完了(= Epic)に向けてドライブする個々のタスクと考えることができます.
